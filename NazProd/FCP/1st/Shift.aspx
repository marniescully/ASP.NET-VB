...
<div id="sideNav">
 ...
 <h3>Tasks</h3>
    ...
    <li><asp:LinkButton ID="ImportProd" runat="server"  PostBackUrl="~/FCP/1st/ImportTMProd.aspx"  onmouseover="window.status='Import Productivity';return true;" onmouseout ="window.status=''; return true;">Import Productivity</asp:LinkButton> <ajaxToolkit:ConfirmButtonExtender ID="ImportConfirmButton" runat="server" TargetControlID="ImportProd" ConfirmText="Click OK only if you have verified the Team Members that worked on the Import Date.">
     ...
     <li><asp:LinkButton ID="EditProd" runat="server" PostBackUrl="~/FCP/1st/Edit.aspx" onmouseover="window.status='Edit Productivity';return true;" onmouseout ="window.status=''; return true;">Edit Productivity</asp:LinkButton> </li>
...
<div id="LeftColumn">
<h2><asp:Label ID="DeptLabel" runat="server" Text="Full Case Pick 1st Shift"></asp:Label> </h2>
  ...
  <asp:GridView ID="ShiftTM" runat="server"  AllowSorting="True" 
    ...
    <div id="Form" class="SmallForm">
     ...
     <asp:SqlDataSource ID="TMDataSource" runat="server" 
          ...
          SelectCommand="SELECT TMs.EmployeeID, TMs.ShiftID, TMs.FirstName, TMs.DepartmentID, TMs.LastName, TMs.FarmIn,  Departments.Dept, Shifts.Shift FROM (Shifts INNER JOIN (Departments INNER JOIN TMs ON Departments.DepartmentID = TMs.DepartmentID) ON Shifts.ShiftID = TMs.ShiftID) WHERE (TMs.ShiftID = 1) AND (TMs.DepartmentID = 2) ORDER BY TMs.EmployeeID" 
      ...
      <asp:Parameter Name="DepartmentID" DbType="Int32" />
       ...
       <asp:Parameter Name="FarmIn" DbType="Boolean" />
        ...
        ConnectionString="<%$ ConnectionStrings:ConnectionString %>" 
        ProviderName="<%$ ConnectionStrings:ConnectionString.ProviderName %>" 
        SelectCommand="SELECT Function, Description, Rate, SumOfAmount,  SumOfFunctHours,  SumOfAmount / SumOfFunctHours AS ActRate,SumOfAmount/Rate AS GoalHours, (SumOfAmount/Rate)/ SumOfFunctHours AS percntGoal FROM SummaryFCP1stDay"></asp:SqlDataSource>
         
          <asp:SqlDataSource ID="PrevWeek" runat="server" 
        ConnectionString="<%$ ConnectionStrings:ConnectionString %>" 
        ProviderName="<%$ ConnectionStrings:ConnectionString.ProviderName %>" SelectCommand="SELECT Function, Rate, Description, SumOfSumOfAmount, SumOfSumOfFunctHours, [SumOfSumOfAmount]/[SumOfSumOfFunctHours] AS ActRate,  SumOfSumOfAmount/Rate AS GoalHours, (SumOfSumOfAmount/Rate)/ SumOfSumOfFunctHours AS percntGoal FROM SummOfSummFCP1stWeek
"></asp:SqlDataSource>

          <asp:SqlDataSource ID="PrevMonth" runat="server" 
        ConnectionString="<%$ ConnectionStrings:ConnectionString %>" 
        ProviderName="<%$ ConnectionStrings:ConnectionString.ProviderName %>" 
        SelectCommand="SELECT Function, Description, Rate, SumOfSumOfAmount,  SumOfSumOfFunctHours, SumOfSumOfAmount / SumOfSumOfFunctHours AS ActRate, SumOfSumOfAmount/Rate AS GoalHours, (SumOfSumOfAmount/Rate)/ SumOfSumOfFunctHours AS percntGoal FROM SummOfSummFCP1stMonth">
</asp:SqlDataSource>
