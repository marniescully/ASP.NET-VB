...
Partial Class ImportTM
    ...
    Protected Sub Page_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
       ...
       ConfigurationManager.ConnectionStrings("ConnectionString").ConnectionString
        ...
        Dim sqlStrLastDate = "SELECT Max(Prod.DTE) AS MaxOfDTE FROM PROD WHERE (((Prod.Department)=8) AND ((Prod.Shift)=2))"
    ...
    Protected Sub ImportButton_OnClick(ByVal sender As Object, ByVal e As System.EventArgs) Handles ImportButton.Click
        ...
        sqlStrFunction = "SELECT Count(Prod.DTE) AS CountOfDTE FROM Prod WHERE (Prod.DTE) = @Date AND (Prod.Department)=8 AND (Prod.Shift)=2"
        ...
        sqlStr = "INSERT INTO Prod ( DTE, M, D, Y, EmployeeID, Amount, Department, Shift, Function ) SELECT DateValue([M] & '/' & [D] & '/' & [Y]) AS [Date], Mid([SCPImportTempTable].[ACTUAL_DATE_TAG_PRINTED],4,2) AS M, RIGHT([SCPImportTempTable].[ACTUAL_DATE_TAG_PRINTED],2) AS D, Mid([SCPImportTempTable].[ACTUAL_DATE_TAG_PRINTED],2,2) AS Y, SCPImportTempTable.RECEIVER_EMPLOYEE_ID, SCPImportTempTable.CountOfPALLET_TAG_NBR,8 AS Department, 2 AS Shift, SCPImportTempTable.SHIPMENT_CHARACTERISTICS FROM TMs INNER JOIN SCPImportTempTable ON TMs.EmployeeID = SCPImportTempTable.RECEIVER_EMPLOYEE_ID WHERE (((TMs.DepartmentID)=8) AND ((TMs.ShiftID)=2)) ORDER BY SCPImportTempTable.RECEIVER_EMPLOYEE_ID;"
