<%@ Page Title="Full Case Pick 2nd Shift Productivity Summary by Function" Language="VB" MasterPageFile="~/Site.master" AutoEventWireup="false" CodeFile="SummbyFunct.aspx.vb" Inherits="FCP" %>
...
<asp:Content ID="Content2" ContentPlaceHolderID="MainContent" Runat="Server">
 ...
 <p class="floatRight">
    <asp:HyperLink ID="backToShiftLink" runat="server" NavigateUrl="~/FCP/2nd/Shift.aspx">2nd Shift Home</asp:HyperLink>
    </p>
 
    <h2>
        Full Case Pick 2nd Shift Productivity Summary by Function
    </h2>
...
<FooterTemplate></table></FooterTemplate>
       ...
       <asp:SqlDataSource ID="FunctListDataSource" runat="server" 
        ...
        SelectCommand="SELECT Function, Description FROM Functions WHERE (Functions.Department)=2 Order by Description">
     </asp:SqlDataSource>
        ...
        ConnectionString="<%$ ConnectionStrings:ConnectionString %>" 
        ProviderName="<%$ ConnectionStrings:ConnectionString.ProviderName %>" 
        SelectCommand=" SELECT DISTINCT Functions.Description, Functions.Function, Functions.Rate FROM Functions INNER JOIN Prod ON (Functions.Department = Prod.Department) AND (Functions.Function = Prod.Function) WHERE (Prod.DTE BETWEEN ? AND ?) AND (Prod.Shift = 2) AND (Prod.Department = 2)">
 ...
 <SelectParameters>
        ...
        ConnectionString="<%$ ConnectionStrings:ConnectionString %>" 
        ProviderName="<%$ ConnectionStrings:ConnectionString.ProviderName %>" 
        SelectCommand=" SELECT DISTINCT Functions.Description, Functions.Function, Functions.Rate FROM Functions INNER JOIN Prod ON (Functions.Department = Prod.Department) AND (Functions.Function = Prod.Function) WHERE (Prod.DTE BETWEEN ? AND ?) AND (Prod.Function=?) AND (Prod.Shift = 2) AND (Prod.Department = 2)">
