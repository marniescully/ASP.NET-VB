...
Partial Class REC
   ...
   Dim sqlStrProdbyTMAll As String = "INSERT INTO SummTempTable ( Department, Rate, SumOfAmount, SumOfTags, SumOfFunctHours,  Description ) SELECT 1 AS Department, Functions.Rate, Sum(Prod.Amount) AS SumOfAmount, Sum(Prod.Tags) AS SumOfTags,Sum(Prod.FunctHours) AS SumOfFunctHours,  Functions.Description FROM Functions  INNER JOIN Prod ON (Functions.Department = Prod.Department) AND (Functions.Function = Prod.Function) WHERE (Prod.Department) =1 GROUP BY Functions.Rate, Functions.Description, Prod.DTE, Prod.Function HAVING (Prod.DTE) Between @Date1 And @Date2"

            Dim sqlStrProdbyTM As String = "INSERT INTO SummTempTable ( Department, Rate, SumOfAmount,SumOfTags, SumOfFunctHours,  Description ) SELECT 1 AS Department, Functions.Rate, Sum(Prod.Amount) AS SumOfAmount, Sum(Prod.Tags) AS SumOfTags, Sum(Prod.FunctHours) AS SumOfFunctHours,  Functions.Description FROM Functions  INNER JOIN Prod ON (Functions.Department = Prod.Department) AND (Functions.Function = Prod.Function) WHERE (Prod.Department) =1 GROUP BY Functions.Rate, Functions.Description, Prod.DTE, Prod.Function HAVING (Prod.DTE) Between @Date1 And @Date2 AND (Prod.Function)=@Funct"

            Dim sqlStrFinal As String = "SELECT SummOfSummRECSummTempTable.Description, SummOfSummRECSummTempTable.Rate, SummOfSummRECSummTempTable.SumOfSumOfAmount AS Amount,  SummOfSummRECSummTempTable.SumOfSumOfTags AS Tags, SummOfSummRECSummTempTable.SumOfSumOfFunctHours AS FunctHours,  [SumOfSumOfAmount]/[SumOfSumOfFunctHours] AS ActualRate, [SumOfSumofAmount]/[Rate] AS GoalHours, ([SumOfSumOfAmount]/[Rate])/[SumOfSumOfFunctHours] AS PercentAvg FROM SummOfSummRECSummTempTable"
