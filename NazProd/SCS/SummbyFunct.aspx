<%@ Page Title="Split Case Stock Productivity Summary by Function" Language="VB" MasterPageFile="~/Site.master" AutoEventWireup="false" CodeFile="SummbyFunct.aspx.vb" Inherits="SCS" %>

<asp:Content ID="Content1" ContentPlaceHolderID="HeadContent" Runat="Server">
</asp:Content>
<asp:Content ID="Content2" ContentPlaceHolderID="MainContent" Runat="Server">
<p class="floatRight">
    <asp:HyperLink ID="backToShiftLink" runat="server" NavigateUrl="~/SCS.aspx">Back to Split Case Stock</asp:HyperLink>
    </p>
 
    <h2>
       Split Case Stock Productivity Summary by Function
    </h2>
...
<FooterTemplate></table></FooterTemplate>
       ...
       <asp:SqlDataSource ID="FunctListDataSource" runat="server" 
        ...
        ConnectionString="<%$ ConnectionStrings:ConnectionString %>" 
        ProviderName="<%$ ConnectionStrings:ConnectionString.ProviderName %>" 
        SelectCommand=" SELECT DISTINCT Functions.Description, Functions.Function, Functions.Rate FROM Functions INNER JOIN Prod ON (Functions.Department = Prod.Department) AND (Functions.Function = Prod.Function) WHERE (Prod.DTE BETWEEN ? AND ?) AND (Prod.Department =9)">
 ...
 <SelectParameters>
        ...
        ConnectionString="<%$ ConnectionStrings:ConnectionString %>" 
        ProviderName="<%$ ConnectionStrings:ConnectionString.ProviderName %>" 
        SelectCommand=" SELECT DISTINCT Functions.Description, Functions.Function, Functions.Rate FROM Functions INNER JOIN Prod ON (Functions.Department = Prod.Department) AND (Functions.Function = Prod.Function) WHERE (Prod.DTE BETWEEN ? AND ?) AND (Prod.Function=?) AND (Prod.Department = 9)">
