...
Partial Class ImportTM
    ...
    Protected Sub Page_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
       ...
       ConfigurationManager.ConnectionStrings("ConnectionString").ConnectionString
        ...
        Dim sqlStrLastDate = "SELECT Max(Prod.DTE) AS MaxOfDTE FROM PROD WHERE (((Prod.Department)=9) AND ((Prod.Shift)=3))"
    ...
    Protected Sub ImportButton_OnClick(ByVal sender As Object, ByVal e As System.EventArgs) Handles ImportButton.Click
        ...
        sqlStrFunction = "SELECT Count(Prod.DTE) AS CountOfDTE FROM Prod WHERE (Prod.DTE) = @Date AND (Prod.Shift)=3 and (Prod.Department) =9"
        ...
        'Tailor this to field names in table and command
        sqlStrCountImport = "SELECT Count(SCSImportTempTable.USER_STAMP) AS CountOfUSER_STAMP FROM TMs INNER JOIN SCSImportTempTable ON TMs.EmployeeID = SCSImportTempTable.USER_STAMP GROUP BY TMs.DepartmentID, TMs.ShiftID HAVING (TMs.DepartmentID)=9 AND (TMs.ShiftID)=3"
