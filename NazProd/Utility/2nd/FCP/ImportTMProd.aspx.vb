...
Partial Class ImportTM
    ...
    Protected Sub Page_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
       ...
       ConfigurationManager.ConnectionStrings("ConnectionString").ConnectionString
        ...
        Dim sqlStrLastDate = "SELECT Max(Prod.DTE) AS MaxOfDTE FROM PROD WHERE (((Prod.Department)=2) AND ((Prod.Shift)=2))"
    ...
    Private Sub ImportFCPMTV()
       ...
       ConfigurationManager.ConnectionStrings("ConnectionString").ConnectionString
        ...
        sqlStrMTV = "INSERT INTO PROD ( DTE, M, D, Y, EmployeeID, Amount, Department, Shift, Function ) SELECT DateValue([M] & '/' & [D] & '/' & [Y]) AS [Date], Mid([FCP-MTVImportTempTable].[PROCESS_CYCLE_DTE],4,2) AS M, RIGHT([FCP-MTVImportTempTable].[PROCESS_CYCLE_DTE],2) AS D, Mid([FCP-MTVImportTempTable].[PROCESS_CYCLE_DTE],2,2) AS Y, FCP-MTVImportTempTable.USER_STAMP, FCP-MTVImportTempTable.CountOfOUTBOUND_CASE_SCAN_ID_ATR, 2 AS Department, 2 AS Shift, FCP-MTVImportTempTable.DISTRIBUTION_CENTER_NBR FROM TMs INNER JOIN FCP-MTVImportTempTable ON TMs.EmployeeID = FCP-MTVImportTempTable.USER_STAMP WHERE (((TMs.DepartmentID)=5) AND ((TMs.ShiftID)=2)) ORDER BY FCP-MTVImportTempTable.USER_STAMP"
    ...
    Private Sub ImportR()
       ...
       ConfigurationManager.ConnectionStrings("ConnectionString").ConnectionString
        ...
        Dim sqlStrFCS As String
        sqlStrFCS = "INSERT INTO Prod (DTE, M, D, Y, EmployeeID, Amount, Department, Shift, Function ) SELECT DateValue([M] & '/' & [D] & '/' & [Y]) AS [Date], Mid([FCPFCSImportTempTable].[RF_TRANSACTION_DATE],4,2) AS M, RIGHT([FCPFCSImportTempTable].[RF_TRANSACTION_DATE],2) AS D, Mid([FCPFCSImportTempTable].[RF_TRANSACTION_DATE],2,2) AS Y, FCPFCSImportTempTable.USER_STAMP, FCPFCSImportTempTable.CountOfRF_TRANS_BAR_CODE, 2 AS Department, 2 AS Shift, 'R' as Function FROM TMs INNER JOIN FCPFCSImportTempTable ON TMs.EmployeeID = FCPFCSImportTempTable.USER_STAMP WHERE (((TMs.DepartmentID)=5) AND ((TMs.ShiftID)=2)) ORDER BY FCPFCSImportTempTable.USER_STAMP"
    ...
    Protected Sub ImportButton_OnClick(ByVal sender As Object, ByVal e As System.EventArgs) Handles ImportButton.Click
       ...
       ConfigurationManager.ConnectionStrings("ConnectionString").ConnectionString
        ...
        sqlStrFunction = "SELECT Count(Prod.DTE) AS CountOfDTE FROM Prod WHERE (Prod.DTE) = @Date AND (Prod.Shift)=2 AND (Prod.Department)=2 AND (((TMs.DepartmentID)=5) AND ((TMs.ShiftID)=2))"
        ...
        sqlStrCountImport = "SELECT Count(Prod.DTE) AS CountOfDTE FROM Prod WHERE (Prod.DTE) = @Date AND (Prod.Shift)=2 AND (Prod.Department)=2"
