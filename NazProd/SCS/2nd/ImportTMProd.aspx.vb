...
Partial Class ImportTM
    ...
    Protected Sub Page_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
       ...
       ConfigurationManager.ConnectionStrings("ConnectionString").ConnectionString
        ...
        Dim sqlStrLastDate = "SELECT Max(Prod.DTE) AS MaxOfDTE FROM PROD WHERE (((Prod.Department)=9) AND ((Prod.Shift)=2))"
    ...
    Protected Sub ImportButton_OnClick(ByVal sender As Object, ByVal e As System.EventArgs) Handles ImportButton.Click
        ...
        sqlStrImport = "SELECT RF_TRANSACTION_DATE, USER_STAMP, COUNT(RF_TRANS_BAR_CODE) AS CountOfRF_TRANS_BAR_CODE, RF_TRANSACTION_TYPE FROM DIPDLIB.DI281P GROUP BY USER_STAMP, RF_TRANSACTION_TYPE, RF_TRANSACTION_DATE HAVING  (RF_TRANSACTION_TYPE <> 'L') AND (RF_TRANSACTION_TYPE <> 'S')  AND (RF_TRANSACTION_DATE =?) ORDER BY USER_STAMP"
        ...
        sqlStrFunction = "SELECT Count(Prod.DTE) AS CountOfDTE FROM Prod WHERE (Prod.DTE) = @Date AND (Prod.Shift)=2 and (Prod.Department) =9"
        ...
        sqlStrCountImport = "SELECT Count(SCSImportTempTable.USER_STAMP) AS CountOfUSER_STAMP FROM TMs INNER JOIN SCSImportTempTable ON TMs.EmployeeID = SCSImportTempTable.USER_STAMP GROUP BY TMs.DepartmentID, TMs.ShiftID HAVING (TMs.DepartmentID)=3 AND (TMs.ShiftID)=2"
