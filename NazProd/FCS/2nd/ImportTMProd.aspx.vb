...
Partial Class ImportTM
    ...
    Protected Sub Page_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
       ...
       ConfigurationManager.ConnectionStrings("ConnectionString").ConnectionString
        ...
        Dim sqlStrLastDate = "SELECT Max(Prod.DTE) AS MaxOfDTE FROM PROD WHERE (((Prod.Department)=3) AND ((Prod.Shift)=2))"
    ...
    Protected Sub ImportButton_OnClick(ByVal sender As Object, ByVal e As System.EventArgs) Handles ImportButton.Click
        ...
        sqlStrFunction = "SELECT Count(Prod.DTE) AS CountOfDTE FROM Prod WHERE (Prod.DTE) = @Date AND (Prod.Shift)=2 and (Prod.Department) =3"
        ...
        sqlStrCountImport = "SELECT Count(FCSImportTempTable.USER_STAMP) AS CountOfUSER_STAMP FROM TMs INNER JOIN FCSImportTempTable ON TMs.EmployeeID = FCSImportTempTable.USER_STAMP GROUP BY TMs.DepartmentID, TMs.ShiftID HAVING (TMs.DepartmentID)=3 AND (TMs.ShiftID)=2"
        ...
        sqlStr = "INSERT INTO Prod (DTE, M, D, Y, EmployeeID, Amount, Department, Shift, Function ) SELECT DateValue([M] & '/' & [D] & '/' & [Y]) AS [Date], Mid([FCSImportTempTable].[RF_TRANSACTION_DATE],4,2) AS M, RIGHT([FCSImportTempTable].[RF_TRANSACTION_DATE],2) AS D, Mid([FCSImportTempTable].[RF_TRANSACTION_DATE],2,2) AS Y, FCSImportTempTable.USER_STAMP, FCSImportTempTable.CountOfRF_TRANS_BAR_CODE, 3 AS Department, 2 AS Shift, FCSImportTempTable.RF_TRANSACTION_TYPE FROM TMs INNER JOIN FCSImportTempTable ON TMs.EmployeeID = FCSImportTempTable.USER_STAMP WHERE (((TMs.DepartmentID)=3) AND ((TMs.ShiftID)=2)) ORDER BY FCSImportTempTable.USER_STAMP;"
