    ...
    Sub GVFunctProd_RowDatabound(ByVal sender As Object, ByVal e As GridViewRowEventArgs)
        ...
        If e.Row.RowType = DataControlRowType.DataRow Then
           ...
           ConfigurationManager.ConnectionStrings("ConnectionString").ConnectionString
            ...
            sqlStrTestImported = "SELECT COUNT(PROD.Function) AS CountOfFunction FROM Prod WHERE (PROD.DTE) = @DTE AND (PROD.EmployeeID) = @EmployeeID AND (PROD.Function) = @Function AND (PROD.Department = 2) AND (PROD.Shift = 1)"
            ...
            sqlAmount = "SELECT Amount, FunctHours, M FROM Prod WHERE (DTE = @DTE) AND (EmployeeID = @EmployeeID) AND (Function = @Function) AND (Shift = 1) AND (Department = 2)"
        ...
        For Each row As GridViewRow In GVFunctProd.Rows
            ...
            If String.IsNullOrEmpty(Amount.Text) Or String.IsNullOrEmpty(FunctHours.Text) Then
                ...
                If String.IsNullOrEmpty(Amount.Text) And String.IsNullOrEmpty(FunctHours.Text) Then
                    ...
                    sqlStrTestDelete = "SELECT COUNT(PROD.Function) AS CountOfFunction FROM Prod WHERE (PROD.DTE) = @DTE AND (PROD.EmployeeID) = @EmployeeID AND (PROD.Function) = @Function AND (PROD.Department = 2) AND (PROD.Shift = 1)"
                    ...
                    sqlStrDelete = "DELETE FROM Prod WHERE (PROD.DTE) = @DTE AND (PROD.EmployeeID) = @EmployeeID AND (PROD.Function) = @Function AND (PROD.Department = 2) AND (PROD.Shift = 1)"
            ...
            Else
                ...
                sqlStrTestInsert = "SELECT COUNT(PROD.Function) AS CountOfFunction FROM Prod WHERE (PROD.DTE) = @DTE AND (PROD.EmployeeID) = @EmployeeID AND (PROD.Function) = @Function AND (PROD.Department = 2) AND (PROD.Shift = 1)"
                ...
                UpdateAmountSqlString = "UPDATE Prod SET Prod.FunctHours=@FunctHours, Prod.Amount = @Amount WHERE ((Prod.DTE) = @DTE And (Prod.EmployeeID) = @EmployeeID And (Prod.Function) = @Function And (Prod.Department) = 2 And (Prod.Shift) = 1)"
