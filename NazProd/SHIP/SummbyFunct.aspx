<%@ Page Title="Shipping Productivity Summary by Function" Language="VB" MasterPageFile="~/Site.master" AutoEventWireup="false" CodeFile="SummbyFunct.aspx.vb" Inherits="SHIP" %>

<asp:Content ID="Content1" ContentPlaceHolderID="HeadContent" Runat="Server">
</asp:Content>
<asp:Content ID="Content2" ContentPlaceHolderID="MainContent" Runat="Server">
<p class="floatRight">
    <asp:HyperLink ID="backToShiftLink" runat="server" NavigateUrl="~/SHIP.aspx">Back to Shipping</asp:HyperLink>
    </p>
 
    <h2>
        Shipping Productivity Summary by Function
    </h2>
...
<FooterTemplate></table></FooterTemplate>
       ...
       <asp:SqlDataSource ID="FunctListDataSource" runat="server" 
        ...
        ConnectionString="<%$ ConnectionStrings:ConnectionString %>" 
        ProviderName="<%$ ConnectionStrings:ConnectionString.ProviderName %>" 
        SelectCommand=" SELECT DISTINCT Functions.Description, Functions.Function, Functions.Rate FROM Functions INNER JOIN Prod ON (Functions.Department = Prod.Department) AND (Functions.Function = Prod.Function) WHERE (Prod.DTE BETWEEN ? AND ?) AND (Prod.Department =4)">
 ...
 <SelectParameters>
        ...
        ConnectionString="<%$ ConnectionStrings:ConnectionString %>" 
        ProviderName="<%$ ConnectionStrings:ConnectionString.ProviderName %>" 
        SelectCommand=" SELECT DISTINCT Functions.Description, Functions.Function, Functions.Rate FROM Functions INNER JOIN Prod ON (Functions.Department = Prod.Department) AND (Functions.Function = Prod.Function) WHERE (Prod.DTE BETWEEN ? AND ?) AND (Prod.Function=?) AND (Prod.Department = 4)">
