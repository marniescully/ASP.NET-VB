...
Partial Class REC
    ...
    Protected Sub Page_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        If Not IsPostBack Then
            ...
            StrtDteCalendarExtender.EndDate = FormatDateTime(SugImpDate, DateFormat.ShortDate)
           ...
           ConfigurationManager.ConnectionStrings("ConnectionString").ConnectionString
            ...
            Dim sqlStrProdbyTMAll As String = "INSERT INTO SummTempTable ( Department, Shift, Rate, SumOfAmount, SumOfTags, SumOfFunctHours, Description ) SELECT 1 AS Department, 3 AS Shift, Functions.Rate, Sum(Prod.Amount) AS SumOfAmount, Sum(Prod.Tags) AS SumOfTags, Sum(Prod.FunctHours) AS SumOfFunctHours,  Functions.Description FROM Functions  INNER JOIN Prod ON (Functions.Department = Prod.Department) AND (Functions.Function = Prod.Function) WHERE(Prod.Shift) = 3 AND (Prod.Department) =1 GROUP BY Functions.Rate, Functions.Description, Prod.DTE, Prod.Function HAVING (Prod.DTE) Between @Date1 And @Date2"

            Dim sqlStrProdbyTM As String = "INSERT INTO SummTempTable ( Department, Shift, Rate, SumOfAmount, SumOfTags, SumOfFunctHours, Description ) SELECT 1 AS Department, 3 AS Shift, Functions.Rate, Sum(Prod.Amount) AS SumOfAmount, Sum(Prod.Tags) AS SumOfTags, Sum(Prod.FunctHours) AS SumOfFunctHours,  Functions.Description FROM Functions  INNER JOIN Prod ON (Functions.Department = Prod.Department) AND (Functions.Function = Prod.Function) WHERE(Prod.Shift) = 3 AND (Prod.Department) =1 GROUP BY Functions.Rate, Functions.Description, Prod.DTE, Prod.Function HAVING (Prod.DTE) Between @Date1 And @Date2 AND (Prod.Function)=@Funct"

            Dim sqlStrFinal As String = "SELECT SummOfSummREC3rdSummTempTable.Description, SummOfSummREC3rdSummTempTable.Rate, SummOfSummREC3rdSummTempTable.SumOfSumOfAmount AS Amount, SummOfSummREC3rdSummTempTable.SumOfSumOfTags AS Tags,SummOfSummREC3rdSummTempTable.SumOfSumOfFunctHours AS FunctHours,  [SumOfSumOfAmount]/[SumOfSumOfFunctHours] AS ActualRate, [SumOfSumofAmount]/[Rate] AS GoalHours, ([SumOfSumOfAmount]/[Rate])/[SumOfSumOfFunctHours] AS PercentAvg FROM SummOfSummREC3rdSummTempTable"
