...
Partial Class FCS_2nd_Add
    ...
    Protected Sub Page_Load(sender As Object, e As System.EventArgs) Handles Me.Load
        ...
        'Stop future of sugimpdate from being selected 
    ...
    Sub GVFunctProd_RowDatabound(ByVal sender As Object, ByVal e As GridViewRowEventArgs)
        ...
        If e.Row.RowType = DataControlRowType.DataRow Then
           ...
           ConfigurationManager.ConnectionStrings("ConnectionString").ConnectionString
            ...
            sqlStrTestImported = "SELECT COUNT(PROD.Function) AS CountOfFunction FROM Prod WHERE (PROD.DTE) = @DTE AND (PROD.EmployeeID) = @EmployeeID AND (PROD.Function) = @Function AND (PROD.Department = 3) AND (PROD.Shift = 2)"
            ...
            sqlAmount = "SELECT Amount, FunctHours, M FROM Prod WHERE (DTE = @DTE) AND (EmployeeID = @EmployeeID) AND (Function = @Function) AND (Shift = 2) AND (Department = 3)"
    ...
    Sub submitClick(sender As Object, e As CommandEventArgs)
        ...
        'if the fields have values, if not check if need to delete, if not move to next row in table
        ...
        For Each row As GridViewRow In GVFunctProd.Rows
            ...
            If String.IsNullOrEmpty(Amount.Text) Or String.IsNullOrEmpty(FunctHours.Text) Then
                ...
                If String.IsNullOrEmpty(Amount.Text) And String.IsNullOrEmpty(FunctHours.Text) Then
                    ...
                    sqlStrTestDelete = "SELECT COUNT(PROD.Function) AS CountOfFunction FROM Prod WHERE (PROD.DTE) = @DTE AND (PROD.EmployeeID) = @EmployeeID AND (PROD.Function) = @Function AND (PROD.Department = 3) AND (PROD.Shift = 2)"
                    ...
                    sqlStrDelete = "DELETE FROM Prod WHERE (PROD.DTE) = @DTE AND (PROD.EmployeeID) = @EmployeeID AND (PROD.Function) = @Function AND (PROD.Department = 3) AND (PROD.Shift = 2)"
            ...
            Else
                ...
                sqlStrTestInsert = "SELECT COUNT(PROD.Function) AS CountOfFunction FROM Prod WHERE (PROD.DTE) = @DTE AND (PROD.EmployeeID) = @EmployeeID AND (PROD.Function) = @Function AND (PROD.Department = 3) AND (PROD.Shift = 2) AND (PROD.M IS NOT NULL)"
                ...
                sqlStrTestInsertM = "SELECT COUNT(PROD.Function) AS CountOfFunction FROM Prod WHERE (PROD.DTE) = @DTE AND (PROD.EmployeeID) = @EmployeeID AND (PROD.Function) = @Function AND (PROD.Department = 3) AND (PROD.Shift = 2) AND (PROD.M IS NULL)"
                ...
                dbCommInsert.Parameters("@Dept").Value = Convert.ToInt32(3)
                dbCommInsert.Parameters.Add("@Shift", OleDbType.Integer)
                dbCommInsert.Parameters("@Shift").Value = Convert.ToInt32(2)
                ...
                UpdateSqlString = "UPDATE Prod SET Prod.FunctHours=@FunctHours WHERE ((Prod.DTE) = @DTE And (Prod.EmployeeID) = @EmployeeID And (Prod.Function) = @Function And (Prod.Department) = 3 And (Prod.Shift) = 2)"
                ...
                UpdateAmountSqlString = "UPDATE Prod SET Prod.FunctHours=@FunctHours, Prod.Amount = @Amount WHERE ((Prod.DTE) = @DTE And (Prod.EmployeeID) = @EmployeeID And (Prod.Function) = @Function And (Prod.Department) = 3 And (Prod.Shift) = 2)"
