...
Partial Class ImportTM
    ...
    Protected Sub Page_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
       ...
       ConfigurationManager.ConnectionStrings("ConnectionString").ConnectionString
        ...
        Dim sqlStrLastDate = "SELECT Max(Prod.DTE) AS MaxOfDTE FROM PROD WHERE (((Prod.Department)=8) AND ((Prod.Shift)=3))"
    ...
    Protected Sub ImportButton_OnClick(ByVal sender As Object, ByVal e As System.EventArgs) Handles ImportButton.Click
        ...
        sqlStrFunction = "SELECT Count(PROD.DTE) AS CountOfDTE FROM PROD WHERE (PROD.DTE) = @Date AND (PROD.Department)=8 AND (PROD.Shift)=3"
        ...
        sqlStr = "INSERT INTO PROD ( DTE, M, D, Y, EmployeeID, Amount, Department, Shift, Function ) SELECT DateValue([M] & '/' & [D] & '/' & [Y]) AS [Date], Mid([SCPImportTempTable].[ACTUAL_DATE_TAG_PRINTED],4,2) AS M, RIGHT([SCPImportTempTable].[ACTUAL_DATE_TAG_PRINTED],2) AS D, Mid([SCPImportTempTable].[ACTUAL_DATE_TAG_PRINTED],2,2) AS Y, SCPImportTempTable.RECEIVER_EMPLOYEE_ID, SCPImportTempTable.CountOfPALLET_TAG_NBR, 8 AS Department, 3 AS Shift, SCPImportTempTable.SHIPMENT_CHARACTERISTICS FROM TMs INNER JOIN SCPImportTempTable ON TMs.EmployeeID = SCPImportTempTable.RECEIVER_EMPLOYEE_ID WHERE (((TMs.DepartmentID)=8) AND ((TMs.ShiftID)=3)) ORDER BY SCPImportTempTable.RECEIVER_EMPLOYEE_ID;"
