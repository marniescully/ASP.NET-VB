<%@ Page Title="Full Case Pick 1st Shift Productivity by Team Member" Language="VB" MasterPageFile="~/Site.master" AutoEventWireup="false" CodeFile="ViewbyTM.aspx.vb" Inherits="FCP" %>
...
<asp:Content ID="Content2" ContentPlaceHolderID="MainContent" Runat="Server">
 ...
 <p class="floatRight">
    <asp:HyperLink ID="backToShiftLink" runat="server" NavigateUrl="~/FCP/1st/Shift.aspx">1st Shift Home</asp:HyperLink>
    </p>
 
    <h2>
        Full Case Pick 1st Shift Productivity by Team Member
    </h2>
  ...
  <div id="SearchReports">
    ...
    <div class="AllRepeat">
        <asp:Repeater ID="DistTM" runat="server" DataSourceID="TMProductofDate" OnItemDataBound="DistTM_ItemDataBound">
            <ItemTemplate> <asp:Label ID="NoDataDistTM" runat="server" Text='No Data to Display' Visible="false" CssClass="Red floatLeft"/> 
                ...
                <table id="TMLabelTable" class="PageBreak">
                    <tr>
                        <td class="left">
                         <h3 class="Red negMarg"><asp:Label ID="EmployeeIDLabel" runat="server" Text='<%# Container.DataItem("FullName")%>' /></h3>
      ...
      <asp:SqlDataSource ID="TMListDataSource" runat="server" 
        ...
        SelectCommand="SELECT EmployeeID,  (FirstName + ' ' + LastName) AS FullName, ShiftID, DepartmentID FROM TMs WHERE ((ShiftID = 1) AND (DepartmentID = 2)) ORDER BY EmployeeID">
     </asp:SqlDataSource>
...
WHERE (((PROD.DTE) Between ? And ?) AND ((TMs.DepartmentID)=2) AND ((TMs.ShiftID)=1))">
 ...
 <SelectParameters>
      ...
      <asp:SqlDataSource ID="TMProductofDateOneFunct" runat="server" 
        ...
        ProviderName="<%$ ConnectionStrings:ConnectionString.ProviderName %>" 
        SelectCommand=" SELECT DISTINCT PROD.EmployeeID, ([TMS].[FirstName] +' '+[TMS].[LastName]) AS FullName FROM TMs INNER JOIN PROD ON TMs.EmployeeID = PROD.EmployeeID
WHERE (((PROD.DTE) Between ? And ?) AND ((TMs.DepartmentID)=2) AND ((TMs.ShiftID)=1)) AND(PROD.Function =? )">
