<%@ Page Title="Split Case Stock 1st Shift Productivity by Team Member" Language="VB" MasterPageFile="~/Site.master" AutoEventWireup="false" CodeFile="ViewbyTM.aspx.vb" Inherits="SCS" %>
...
<asp:Content ID="Content2" ContentPlaceHolderID="MainContent" Runat="Server">
 ...
 <p class="floatRight">
    ...
    <h2>
        Split Case Stock 1st Shift Productivity by Team Member
    </h2>
...
<div class="AllRepeat">
   ...
   <asp:Label ID="NoData" runat="server" Text="No Data to Display" Visible="false" CssClass="Red"></asp:Label>
     ...
     <asp:SqlDataSource ID="TMListDataSource" runat="server" 
        ...
        SelectCommand="SELECT EmployeeID,  (FirstName + ' ' + LastName) AS FullName, ShiftID, DepartmentID FROM TMs WHERE ((ShiftID = 1) AND (DepartmentID = 9)) ORDER BY EmployeeID">
     </asp:SqlDataSource>
...
WHERE (((PROD.DTE) Between ? And ?) AND ((TMs.DepartmentID)=9) AND ((TMs.ShiftID)=1))">
 ...
 <SelectParameters>
        ...
        ProviderName="<%$ ConnectionStrings:ConnectionString.ProviderName %>" 
        SelectCommand=" SELECT DISTINCT PROD.EmployeeID, ([TMS].[FirstName] +' '+[TMS].[LastName]) AS FullName FROM TMs INNER JOIN PROD ON TMs.EmployeeID = PROD.EmployeeID
WHERE (((PROD.DTE) Between ? And ?) AND (PROD.Function =? )  AND ((TMs.DepartmentID)=9) AND ((TMs.ShiftID)=1))">
