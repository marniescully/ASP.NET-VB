<%@ Page Title="Receiving 1st Shift Productivity by Function" Language="VB" MasterPageFile="~/Site.master" AutoEventWireup="false" CodeFile="ViewbyFunct.aspx.vb" Inherits="REC" %>
...
<asp:Content ID="Content2" ContentPlaceHolderID="MainContent" runat="Server">
    ...
    <h2>
        Receiving 1st Shift Productivity by Function
    </h2>
  ...
  <div id="SearchReports">
        ...
        <asp:DropDownList ID="FunctList" runat="server" DataTextField="Description" DataValueField="Function"
            DataSourceID="FunctListDataSource" AppendDataBoundItems="true" Class="DropDownList">
...
<asp:Label ID="NoData" runat="server" Text="No Data to Display" Visible="false" CssClass="Red" ></asp:Label>
    ...
    <asp:SqlDataSource ID="DatesDataSource" runat="server" ConnectionString="<%$ ConnectionStrings:ConnectionString %>"
        ProviderName="<%$ ConnectionStrings:ConnectionString.ProviderName %>" SelectCommand="SELECT DISTINCT DTE FROM PROD WHERE DTE Between ? And ? AND Shift=1 and Department =1">
        <SelectParameters>
            <asp:ControlParameter ControlID="txtStartDate" Name="?" PropertyName="Text" ConvertEmptyStringToNull="False"
                Type="DateTime" />
            <asp:ControlParameter ControlID="txtEndDate" Name="?" PropertyName="Text" Type="DateTime" />
        </SelectParameters>
    </asp:SqlDataSource>
    <asp:SqlDataSource ID="ProductFunctofDateOne" runat="server" ConnectionString="<%$ ConnectionStrings:ConnectionString %>"
        ProviderName="<%$ ConnectionStrings:ConnectionString.ProviderName %>" SelectCommand=" SELECT DISTINCT Functions.Description, Functions.Function, Functions.Rate FROM Functions INNER JOIN Prod ON (Functions.Department = Prod.Department) AND (Functions.Function = Prod.Function)
WHERE (Prod.DTE BETWEEN ? AND ?) AND (Prod.Function=?) AND (Prod.Department = 1) AND (Prod.Shift = 1)">
        <SelectParameters>
