<%@ Page Title="Receiving 3rd Shift Productivity by Team Member" Language="VB" MasterPageFile="~/Site.master"
    AutoEventWireup="false" CodeFile="ViewbyTM.aspx.vb" Inherits="REC" %>
...
<asp:Content ID="Content2" ContentPlaceHolderID="MainContent" runat="Server">
 ...
 <p class="floatRight">
    <asp:HyperLink ID="backToShiftLink" runat="server" NavigateUrl="~/REC/3rd/Shift.aspx">3rd Shift Home</asp:HyperLink>
    </p>
 
    <h2>
        Receiving 3rd Shift Productivity by Team Member
    </h2>
  ...
  <div id="SearchReports">
   ...
   <div class="AllRepeat">
    ...
    <asp:SqlDataSource ID="TMListDataSource" runat="server" ConnectionString="<%$ ConnectionStrings:ConnectionString %>"
        ProviderName="<%$ ConnectionStrings:ConnectionString.ProviderName %>" SelectCommand="SELECT EmployeeID,  (FirstName + ' ' + LastName) AS FullName, ShiftID, DepartmentID FROM TMs WHERE ((ShiftID = 3) AND (DepartmentID = 1)) ORDER BY EmployeeID">
    </asp:SqlDataSource>

    <asp:SqlDataSource ID="FunctListDataSource" runat="server" ConnectionString="<%$ ConnectionStrings:ConnectionString %>"
        ProviderName="<%$ ConnectionStrings:ConnectionString.ProviderName %>" SelectCommand="SELECT Function, Description FROM Functions WHERE Functions.Department =1 Order by Description">
    </asp:SqlDataSource>
...
WHERE (((PROD.DTE) Between ? And ?) AND ((TMs.DepartmentID)=1) AND ((TMs.ShiftID)=3))">
        <SelectParameters>
    ...
    <asp:SqlDataSource ID="TMProductofDateOneFunct" runat="server" ConnectionString="<%$ ConnectionStrings:ConnectionString %>"
        ProviderName="<%$ ConnectionStrings:ConnectionString.ProviderName %>" SelectCommand=" SELECT DISTINCT PROD.EmployeeID, ([TMS].[FirstName]+' '+[TMS].[LastName]) AS FullName FROM TMs INNER JOIN PROD ON TMs.EmployeeID = PROD.EmployeeID
WHERE (((PROD.DTE) Between ? And ?) AND ((TMs.DepartmentID)=1) AND ((TMs.ShiftID)=3) AND ((PROD.Function)=?))">
        <SelectParameters>
            <asp:ControlParameter ControlID="txtStartDate" DbType="Date" Name="?" PropertyName="Text" />
            <asp:ControlParameter ControlID="txtEndDate" DbType="Date" Name="?" PropertyName="Text" />
