    ...
    Protected Sub Page_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        If Not IsPostBack Then
           ...
           ConfigurationManager.ConnectionStrings("ConnectionString").ConnectionString
            ...
            Dim sqlStrProdbyTMAll As String = "INSERT INTO SummTempTable ( Department, Rate, SumOfAmount, SumOfFunctHours, SumOfMiscHours, Description ) SELECT 8 AS Department, Functions.Rate, Sum(Prod.Amount) AS SumOfAmount, Sum(Prod.FunctHours) AS SumOfFunctHours, Sum() AS SumOfMiscHours, Functions.Description FROM Functions  INNER JOIN Prod ON (Functions.Department = Prod.Department) AND (Functions.Function = Prod.Function) WHERE (Prod.Department) =8 GROUP BY Functions.Rate, Functions.Description, Prod.DTE, Prod.Function HAVING (Prod.DTE) Between @Date1 And @Date2"

            Dim sqlStrProdbyTM As String = "INSERT INTO SummTempTable ( Department, Rate, SumOfAmount, SumOfFunctHours, SumOfMiscHours, Description ) SELECT 8 AS Department, Functions.Rate, Sum(Prod.Amount) AS SumOfAmount, Sum(Prod.FunctHours) AS SumOfFunctHours, Sum() AS SumOfMiscHours, Functions.Description FROM Functions  INNER JOIN Prod ON (Functions.Department = Prod.Department) AND (Functions.Function = Prod.Function) WHERE (Prod.Department) =8 GROUP BY Functions.Rate, Functions.Description, Prod.DTE, Prod.Function HAVING (Prod.DTE) Between @Date1 And @Date2 AND (Prod.Function)=@Funct"

            Dim sqlStrFinal As String = "SELECT SummOfSummSCPSummTempTable.Description, SummOfSummSCPSummTempTable.Rate, SummOfSummSCPSummTempTable.SumOfSumOfAmount AS Amount, SummOfSummSCPSummTempTable.SumOfSumOfFunctHours AS FunctHours, SummOfSummSCPSummTempTable.SumOfSumOfMiscHours AS MiscHours, [SumOfSumOfAmount]/[SumOfSumOfFunctHours] AS ActualRate, [SumOfSumofAmount]/[Rate] AS GoalHours, ([SumOfSumOfAmount]/[Rate])/[SumOfSumOfFunctHours] AS PercentAvg, ([SumOfSumOfFunctHours]+[SumOfSumOfMiscHours]) AS TotalHours FROM SummOfSummSCPSummTempTable"
