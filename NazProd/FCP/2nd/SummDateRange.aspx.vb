    ...
    Protected Sub Page_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        If Not IsPostBack Then
           ...
           ConfigurationManager.ConnectionStrings("ConnectionString").ConnectionString
            ...
            Dim sqlStrProdbyTMAll As String = "INSERT INTO SummTempTable ( Department, Shift, Rate, SumOfAmount, SumOfFunctHours,  Description ) SELECT 2 AS Department, 2 AS Shift, Functions.Rate, Sum(Prod.Amount) AS SumOfAmount, Sum(Prod.FunctHours) AS SumOfFunctHours,  Functions.Description FROM Functions  INNER JOIN Prod ON (Functions.Department = Prod.Department) AND (Functions.Function = Prod.Function) WHERE(Prod.Shift) = 2 AND (Prod.Department) =2 GROUP BY Functions.Rate, Functions.Description, Prod.DTE, Prod.Function HAVING (Prod.DTE) Between @Date1 And @Date2"

            Dim sqlStrProdbyTM As String = "INSERT INTO SummTempTable ( Department, Shift, Rate, SumOfAmount, SumOfFunctHours,  Description ) SELECT 2 AS Department, 2 AS Shift, Functions.Rate, Sum(Prod.Amount) AS SumOfAmount, Sum(Prod.FunctHours) AS SumOfFunctHours, Functions.Description FROM Functions  INNER JOIN Prod ON (Functions.Department = Prod.Department) AND (Functions.Function = Prod.Function) WHERE(Prod.Shift) = 2 AND (Prod.Department) =2 GROUP BY Functions.Rate, Functions.Description, Prod.DTE, Prod.Function HAVING (Prod.DTE) Between @Date1 And @Date2 AND (Prod.Function)=@Funct"

            Dim sqlStrFinal As String = "SELECT SummOfSummFCP2ndSummTempTable.Description, SummOfSummFCP2ndSummTempTable.Rate, SummOfSummFCP2ndSummTempTable.SumOfSumOfAmount AS Amount, SummOfSummFCP2ndSummTempTable.SumOfSumOfFunctHours AS FunctHours, [SumOfSumOfAmount]/[SumOfSumOfFunctHours] AS ActualRate, [SumOfSumofAmount]/[Rate] AS GoalHours, ([SumOfSumOfAmount]/[Rate])/[SumOfSumOfFunctHours] AS PercentAvg FROM SummOfSummFCP2ndSummTempTable"
