...
Partial Class ImportTM
    ...
    Protected Sub ImportButton_OnClick(ByVal sender As Object, ByVal e As System.EventArgs) Handles ImportButton.Click
        ...
        sqlStrFunction = "SELECT Count(Prod.DTE) AS CountOfDTE FROM Prod WHERE (Prod.DTE) = @Date AND (Prod.Shift)=1 AND (Prod.Department)=4"
        ...
        sqlStrCountImport = "SELECT Count(SHIPImportTempTable.USER_STAMP) AS CountOfUSER_STAMP FROM TMs INNER JOIN SHIPImportTempTable ON TMs.EmployeeID = SHIPImportTempTable.USER_STAMP GROUP BY TMs.DepartmentID, TMs.ShiftID HAVING(TMs.DepartmentID) = 5 And (TMs.ShiftID) = 1"
