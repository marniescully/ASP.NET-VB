    ...
    Public Sub DateRepeat_ItemDataBound(ByVal sender As Object, ByVal e As System.Web.UI.WebControls.RepeaterItemEventArgs) Handles DateRepeat.ItemDataBound
        ...
        If e.Item.ItemType = ListItemType.Item OrElse e.Item.ItemType = ListItemType.AlternatingItem Then
           ...
           ConfigurationManager.ConnectionStrings("ConnectionString").ConnectionString
            ...
            Dim sqlStrEmpCount As String = "SELECT Count(PROD.EmployeeID) AS CountOfEmployeeID FROM PROD WHERE (PROD.DTE=@Date1) AND (PROD.Function = @Funct)  AND (PROD.Department = 4)  AND (PROD.Shift = 3)"

            Dim sqlStrProductFunctofDate As String = "SELECT DISTINCT Functions.Description, Functions.Function, Functions.Rate FROM Functions INNER JOIN Prod ON (Functions.Department = Prod.Department) AND (Functions.Function = Prod.Function) WHERE (PROD.DTE=@Date1) AND (PROD.Department = 4) AND (PROD.Shift = 3)"
    ...
    Public Sub DistFunct_ItemDataBound(ByVal sender As Object, ByVal e As System.Web.UI.WebControls.RepeaterItemEventArgs) Handles DistFunct.ItemDataBound
        ...
        If e.Item.ItemType = ListItemType.Item OrElse e.Item.ItemType = ListItemType.AlternatingItem Then
           ...
           ConfigurationManager.ConnectionStrings("ConnectionString").ConnectionString
            ...
            Dim sqlStringProd As String = "SELECT  (TMS.FirstName + ' ' + TMS.LastName) AS FullName, Prod.DTE, Functions.Description, Prod.EmployeeID, Prod.Function, Prod.Amount, Prod.FunctHours,  Functions.Rate, PROD.Amount/PROD.FunctHours AS ActualRate, PROD.Amount/Functions.Rate AS GoalHours, PROD.Amount/Functions.Rate/PROD.FunctHours AS PercentAvg, Prod.Shift FROM TMs INNER JOIN (Functions INNER JOIN Prod ON (Functions.Department = Prod.Department) AND (Functions.Function = Prod.Function)) ON TMs.EmployeeID = Prod.EmployeeID WHERE (((Prod.DTE)=[@Date1]) AND ((Prod.Function)=[@Funct]) AND ((Prod.Shift)=3) AND ((Prod.Department)=4))ORDER BY Prod.DTE, Functions.Description, Prod.EmployeeID"
