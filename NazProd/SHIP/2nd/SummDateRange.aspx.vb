    ...
    Protected Sub Page_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        If Not IsPostBack Then
           ...
           ConfigurationManager.ConnectionStrings("ConnectionString").ConnectionString
            ...
            Dim sqlStrProdbyTMAll As String = "INSERT INTO SummTempTable ( Department, Shift, Rate, SumOfAmount, SumOfFunctHours,  Description ) SELECT 4 AS Department, 2 AS Shift, Functions.Rate, Sum(Prod.Amount) AS SumOfAmount, Sum(Prod.FunctHours) AS SumOfFunctHours, Functions.Description FROM Functions  INNER JOIN Prod ON (Functions.Department = Prod.Department) AND (Functions.Function = Prod.Function) WHERE(Prod.Shift) = 2 AND (Prod.Department) =4 GROUP BY Functions.Rate, Functions.Description, Prod.DTE, Prod.Function HAVING (Prod.DTE) Between @Date1 And @Date2"

            Dim sqlStrProdbyTM As String = "INSERT INTO SummTempTable ( Department, Shift, Rate, SumOfAmount, SumOfFunctHours,  Description ) SELECT 4 AS Department, 2 AS Shift, Functions.Rate, Sum(Prod.Amount) AS SumOfAmount, Sum(Prod.FunctHours) AS SumOfFunctHours, Functions.Description FROM Functions  INNER JOIN Prod ON (Functions.Department = Prod.Department) AND (Functions.Function = Prod.Function) WHERE(Prod.Shift) = 2 AND (Prod.Department) =4 GROUP BY Functions.Rate, Functions.Description, Prod.DTE, Prod.Function HAVING (Prod.DTE) Between @Date1 And @Date2 AND (Prod.Function)=@Funct"

            Dim sqlStrFinal As String = "SELECT SummOfSummSHIP2ndSummTempTable.Description, SummOfSummSHIP2ndSummTempTable.Rate, SummOfSummSHIP2ndSummTempTable.SumOfSumOfAmount AS Amount, SummOfSummSHIP2ndSummTempTable.SumOfSumOfFunctHours AS FunctHours,  [SumOfSumOfAmount]/[SumOfSumOfFunctHours] AS ActualRate, [SumOfSumofAmount]/[Rate] AS GoalHours, ([SumOfSumOfAmount]/[Rate])/[SumOfSumOfFunctHours] AS PercentAvg FROM SummOfSummSHIP2ndSummTempTable"
