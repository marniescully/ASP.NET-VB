<%@ Page Title="Full Case Stock 1st Shift" Language="VB" MasterPageFile="~/Site.master" AutoEventWireup="false" CodeFile="Shift.aspx.vb" Inherits="FCS" %>
...
<div id="sideNav">
 ...
 <h3>Tasks</h3>
    ...
    <li><asp:LinkButton ID="ImportProd" runat="server"  PostBackUrl="~/FCS/1st/ImportTMProd.aspx"  onmouseover="window.status='Import Productivity';return true;" onmouseout ="window.status=''; return true;">Import Productivity</asp:LinkButton> <ajaxToolkit:ConfirmButtonExtender ID="ImportConfirmButton" runat="server" TargetControlID="ImportProd" ConfirmText="Click OK only if you have verified the Team Members that worked on the Import Date.">
         </ajaxToolkit:ConfirmButtonExtender> </li>
     <li><asp:LinkButton ID="EditProd" runat="server" PostBackUrl="~/FCS/1st/Edit.aspx" onmouseover="window.status='Edit Productivity';return true;" onmouseout ="window.status=''; return true;">Edit Productivity</asp:LinkButton> </li>
    ...
    <h3>Reports</h3>
        <ul>   
    <li><asp:LinkButton ID="ViewByTM" runat="server" PostBackUrl="~/FCS/1st/ViewbyTM.aspx" onmouseover="window.status='View By Team Member';return true;" onmouseout ="window.status=''; return true;">View by Team Member</asp:LinkButton> </li>
    <li><asp:LinkButton ID="ViewByFunction" runat="server" PostBackUrl="~/FCS/1st/ViewbyFunct.aspx" onmouseover="window.status='View By Function';return true;" onmouseout ="window.status=''; return true;" >View by Function</asp:LinkButton> </li>
    <li><asp:LinkButton ID="SummaryDateRange" runat="server" PostBackUrl="~/FCS/1st/SummDateRange.aspx" onmouseover="window.status='Summary of Date Range';return true;" onmouseout ="window.status=''; return true;">Summary of Date Range</asp:LinkButton> </li>
    <li><asp:LinkButton ID="SummbyFunct" runat="server" PostBackUrl="~/FCS/1st/SummbyFunct.aspx" onmouseover="window.status='Summary by Function';return true;" onmouseout ="window.status=''; return true;">Summary by Function</asp:LinkButton> </li>
    </ul>
       </asp:Panel>
...
<div id="LeftColumn">
<h2><asp:Label ID="DeptLabel" runat="server" Text="Full Case Stock 1st Shift"></asp:Label> </h2>
  ...
  <asp:GridView ID="ShiftTM" runat="server"  AllowSorting="True" 
    ...
    <div id="Form" class="SmallForm">
     ...
     <asp:SqlDataSource ID="TMDataSource" runat="server" 
          ...
          SelectCommand="SELECT TMs.EmployeeID, TMs.ShiftID, TMs.FirstName, TMs.DepartmentID, TMs.LastName, TMs.FarmIn,  Departments.Dept, Shifts.Shift FROM (Shifts INNER JOIN (Departments INNER JOIN TMs ON Departments.DepartmentID = TMs.DepartmentID) ON Shifts.ShiftID = TMs.ShiftID) WHERE (TMs.ShiftID = 1) AND (TMs.DepartmentID = 3) ORDER BY TMs.EmployeeID" 
      ...
      <asp:Parameter Name="DepartmentID" DbType="Int32" />
       ...
       <asp:Parameter Name="FarmIn" DbType="Boolean" />
        ...
        ConnectionString="<%$ ConnectionStrings:ConnectionString %>" 
        ProviderName="<%$ ConnectionStrings:ConnectionString.ProviderName %>" 
        SelectCommand="SELECT Function, Description, Rate, SumOfAmount,  SumOfFunctHours,  SumOfAmount / SumOfFunctHours AS ActRate,SumOfAmount/Rate AS GoalHours, (SumOfAmount/Rate)/ SumOfFunctHours AS percntGoal FROM SummaryFCS1stDay"></asp:SqlDataSource>
         
          <asp:SqlDataSource ID="PrevWeek" runat="server" 
        ConnectionString="<%$ ConnectionStrings:ConnectionString %>" 
        ProviderName="<%$ ConnectionStrings:ConnectionString.ProviderName %>" SelectCommand="SELECT Function, Rate, Description, SumOfSumOfAmount, SumOfSumOfFunctHours, [SumOfSumOfAmount]/[SumOfSumOfFunctHours] AS ActRate,  SumOfSumOfAmount/Rate AS GoalHours, (SumOfSumOfAmount/Rate)/ SumOfSumOfFunctHours AS percntGoal FROM SummOfSummFCS1stWeek
"></asp:SqlDataSource>

          <asp:SqlDataSource ID="PrevMonth" runat="server" 
        ConnectionString="<%$ ConnectionStrings:ConnectionString %>" 
        ProviderName="<%$ ConnectionStrings:ConnectionString.ProviderName %>" 
        SelectCommand="SELECT Function, Description, Rate, SumOfSumOfAmount,  SumOfSumOfFunctHours, SumOfSumOfAmount / SumOfSumOfFunctHours AS ActRate, SumOfSumOfAmount/Rate AS GoalHours, (SumOfSumOfAmount/Rate)/ SumOfSumOfFunctHours AS percntGoal FROM SummOfSummFCS1stMonth">
</asp:SqlDataSource>
