<%@ Page Title="Shipping 2nd Shift Productivity by Team Member" Language="VB" MasterPageFile="~/Site.master" AutoEventWireup="false" CodeFile="ViewbyTM.aspx.vb" Inherits="SHIP" %>
...
<p class="floatRight">
    <asp:HyperLink ID="backToShiftLink" runat="server" NavigateUrl="~/SHIP/2nd/Shift.aspx">2nd Shift Home</asp:HyperLink>
    </p>
 
    <h2>
        Shipping 2nd Shift Productivity by Team Member
    </h2>

  <div id="SearchReports">
        <asp:Label ID="TmListLabel" runat="server">TM:</asp:Label>
        <asp:DropDownList ID="TMList" runat="server"  DataTextField="FullName" DataValueField="EmployeeID"  DataSourceID="TMListDataSource" AppendDataBoundItems="true"  Class="DropDownList" >
         <asp:ListItem Value="All" Selected="True">All </asp:ListItem>
           </asp:DropDownList>

            <asp:DropDownList ID="TMListbyName" runat="server" DataTextField="FullName" DataValueField="EmployeeID"   Visible="false" Class="DropDownList"  AppendDataBoundItems="false">
       
           </asp:DropDownList>

           <asp:Label ID="FunctionLabel" runat="server">Function: &nbsp;</asp:Label>
        <asp:DropDownList ID="FunctList" runat="server" Class="DropDownList"   DataTextField="Description" DataValueField="Function"  DataSourceID="FunctListDataSource" AppendDataBoundItems="true">
    <asp:ListItem Value="All" Selected="True">All </asp:ListItem>
       </asp:DropDownList>
...
<div class="AllRepeat">
 ...
 <asp:Label ID="NoData" runat="server" Text="No Data to Display" Visible="false" CssClass="Red"></asp:Label>
     ...
     <asp:SqlDataSource ID="TMListDataSource" runat="server" 
        ...
        SelectCommand="SELECT EmployeeID,  (FirstName + ' ' + LastName) AS FullName, ShiftID, DepartmentID FROM TMs WHERE ((ShiftID = 2) AND (DepartmentID = 4)) ORDER BY EmployeeID">
     </asp:SqlDataSource>

       <asp:SqlDataSource ID="FunctListDataSource" runat="server" 
        ConnectionString="<%$ ConnectionStrings:ConnectionString %>" 
        ProviderName="<%$ ConnectionStrings:ConnectionString.ProviderName %>" 
        SelectCommand="SELECT Function, Description FROM Functions WHERE Functions.Department =4 Order by Description">
     </asp:SqlDataSource>
...
WHERE (((PROD.DTE) Between ? And ?) AND ((TMs.DepartmentID)=4) AND ((TMs.ShiftID)=2))">
 ...
 <SelectParameters>
        ...
        ProviderName="<%$ ConnectionStrings:ConnectionString.ProviderName %>" 
        SelectCommand=" SELECT DISTINCT PROD.EmployeeID, ([TMS].[FirstName] +' '+[TMS].[LastName]) AS FullName FROM TMs INNER JOIN PROD ON TMs.EmployeeID = PROD.EmployeeID
WHERE (((PROD.DTE) Between ? And ?) AND ((TMs.DepartmentID)=4) AND ((TMs.ShiftID)=2)) AND (PROD.Function =? )">
