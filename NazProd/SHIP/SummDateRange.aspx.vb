    ...
    Protected Sub Page_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        If Not IsPostBack Then
           ...
           ConfigurationManager.ConnectionStrings("ConnectionString").ConnectionString
            ...
            Dim sqlStrProdbyTMAll As String = "INSERT INTO SummTempTable ( Department, Rate, SumOfAmount, SumOfFunctHours, Description ) SELECT 4 AS Department, Functions.Rate, Sum(Prod.Amount) AS SumOfAmount, Sum(Prod.FunctHours) AS SumOfFunctHours, Functions.Description FROM Functions  INNER JOIN Prod ON (Functions.Department = Prod.Department) AND (Functions.Function = Prod.Function) WHERE (Prod.Department) =4 GROUP BY Functions.Rate, Functions.Description, Prod.DTE, Prod.Function HAVING (Prod.DTE) Between @Date1 And @Date2"

            Dim sqlStrProdbyTM As String = "INSERT INTO SummTempTable ( Department, Rate, SumOfAmount, SumOfFunctHours,  Description ) SELECT 4 AS Department, Functions.Rate, Sum(Prod.Amount) AS SumOfAmount, Sum(Prod.FunctHours) AS SumOfFunctHours,  Functions.Description FROM Functions  INNER JOIN Prod ON (Functions.Department = Prod.Department) AND (Functions.Function = Prod.Function) WHERE (Prod.Department) =4 GROUP BY Functions.Rate, Functions.Description, Prod.DTE, Prod.Function HAVING (Prod.DTE) Between @Date1 And @Date2 AND (Prod.Function)=@Funct"

            Dim sqlStrFinal As String = "SELECT SummOfSummSHIPSummTempTable.Description, SummOfSummSHIPSummTempTable.Rate, SummOfSummSHIPSummTempTable.SumOfSumOfAmount AS Amount, SummOfSummSHIPSummTempTable.SumOfSumOfFunctHours AS FunctHours,  [SumOfSumOfAmount]/[SumOfSumOfFunctHours] AS ActualRate, [SumOfSumofAmount]/[Rate] AS GoalHours, ([SumOfSumOfAmount]/[Rate])/[SumOfSumOfFunctHours] AS PercentAvg FROM SummOfSummSHIPSummTempTable"
