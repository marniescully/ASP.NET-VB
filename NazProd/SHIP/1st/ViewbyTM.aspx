<%@ Page Title=" Shipping 1st Shift Productivity by Team Member" Language="VB" MasterPageFile="~/Site.master"
    AutoEventWireup="false" CodeFile="ViewbyTM.aspx.vb" Inherits="SHIP" %>

<asp:Content ID="Content1" ContentPlaceHolderID="HeadContent" runat="Server">
</asp:Content>
<asp:Content ID="Content2" ContentPlaceHolderID="MainContent" runat="Server">
    <p class="floatRight">
        <asp:HyperLink ID="backToShiftLink" runat="server" NavigateUrl="~/SHIP/1st/Shift.aspx">1st Shift Home</asp:HyperLink>
    </p>
    <h2>
        Shipping 1st Shift Productivity by Team Member
    </h2>
    <div id="SearchReports">
        <asp:Label ID="TmListLabel" runat="server">TM: </asp:Label>
        <asp:DropDownList ID="TMList" runat="server" DataTextField="FullName" DataValueField="EmployeeID"
            DataSourceID="TMListDataSource" AppendDataBoundItems="true" Class="DropDownList">
            <asp:ListItem Value="All" Selected="True">All </asp:ListItem>
        </asp:DropDownList>
        <asp:DropDownList ID="TMListbyName" runat="server" DataTextField="FullName" DataValueField="EmployeeID"
            Visible="false" Class="DropDownList" AppendDataBoundItems="false">
        </asp:DropDownList>
        <asp:Label ID="FunctionLabel" runat="server">Function: &nbsp;</asp:Label>
        <asp:DropDownList ID="FunctList" runat="server" Class="DropDownList" DataTextField="Description"
            DataValueField="Function" DataSourceID="FunctListDataSource" AppendDataBoundItems="true">
            <asp:ListItem Value="All" Selected="True">All </asp:ListItem>
        </asp:DropDownList>
        ...
        <asp:RequiredFieldValidator ID="StDateRequiredFieldValidator" runat="server" ErrorMessage="The Start Date is a Required field."
        ...
        <asp:RequiredFieldValidator ID="EndDateRequiredFieldValidator" runat="server" ErrorMessage="The End Date is a Required field."
        ...
        <ajaxToolkit:CalendarExtender ID="StDateCalendarExtender" TargetControlID="txtStartDate"
            runat="server">
        </ajaxToolkit:CalendarExtender>
        <ajaxToolkit:CalendarExtender ID="EndDateCalendarExtender" TargetControlID="txtEndDate"
            runat="server">
        </ajaxToolkit:CalendarExtender>
        ...
        <asp:Label ID="TMNotFound" runat="server" Text="" CssClass="bold"></asp:Label>
    </div>
    <div class="AllRepeat">
        <asp:Repeater ID="DistTM" runat="server" DataSourceID="TMProductofDate" OnItemDataBound="DistTM_ItemDataBound">
            <ItemTemplate>
                <asp:Label ID="NoDataDistTM" runat="server" Text='No Data to Display' Visible="false" CssClass="Red floatLeft" />
    ...
    <asp:SqlDataSource ID="TMListDataSource" runat="server" ConnectionString="<%$ ConnectionStrings:ConnectionString %>"
        ProviderName="<%$ ConnectionStrings:ConnectionString.ProviderName %>" SelectCommand="SELECT EmployeeID,  (FirstName + ' ' + LastName) AS FullName, ShiftID, DepartmentID FROM TMs WHERE ((ShiftID = 1) AND (DepartmentID = 4)) ORDER BY EmployeeID">
    </asp:SqlDataSource>
    <asp:SqlDataSource ID="FunctListDataSource" runat="server" ConnectionString="<%$ ConnectionStrings:ConnectionString %>"
        ProviderName="<%$ ConnectionStrings:ConnectionString.ProviderName %>" SelectCommand="SELECT Function, Description FROM Functions WHERE Functions.Department =4 Order by Description">
    </asp:SqlDataSource>
    <asp:SqlDataSource ID="TMProductofDate" runat="server" ConnectionString="<%$ ConnectionStrings:ConnectionString %>"
        ProviderName="<%$ ConnectionStrings:ConnectionString.ProviderName %>" SelectCommand=" SELECT DISTINCT PROD.EmployeeID, ([TMS].[FirstName] +' '+[TMS].[LastName]) AS FullName FROM TMs INNER JOIN PROD ON TMs.EmployeeID = PROD.EmployeeID
WHERE (((PROD.DTE) Between ? And ?) AND ((TMs.DepartmentID)=4) AND ((TMs.ShiftID)=1))">
        <SelectParameters>
    ...
    <asp:SqlDataSource ID="TMProductofDateOneFunct" runat="server" ConnectionString="<%$ ConnectionStrings:ConnectionString %>"
        ProviderName="<%$ ConnectionStrings:ConnectionString.ProviderName %>" SelectCommand=" SELECT DISTINCT PROD.EmployeeID, ([TMS].[FirstName] +' '+[TMS].[LastName]) AS FullName FROM TMs INNER JOIN PROD ON TMs.EmployeeID = PROD.EmployeeID
WHERE (((PROD.DTE) Between ? And ?) AND ((TMs.DepartmentID)=4) AND ((TMs.ShiftID)=1)) AND (PROD.Function =? )">
        <SelectParameters>
            <asp:ControlParameter ControlID="txtStartDate" DbType="Date" Name="?" PropertyName="Text" />
            <asp:ControlParameter ControlID="txtEndDate" DbType="Date" Name="?" PropertyName="Text" />
    ...
    <asp:SqlDataSource ID="TMListName" runat="server" ConnectionString="<%$ ConnectionStrings:ConnectionString %>"
        ProviderName="<%$ ConnectionStrings:ConnectionString.ProviderName %>" SelectCommand="SELECT EmployeeID, (FirstName + ' ' + LastName) AS FullName FROM TMs WHERE (EmployeeID = ?)">
        <SelectParameters>
            <asp:ControlParameter ControlID="TMList" Name="?" PropertyName="SelectedValue" />
        </SelectParameters>
    </asp:SqlDataSource>
    <asp:SqlDataSource ID="TMListByLastName" runat="server" ConnectionString="<%$ ConnectionStrings:ConnectionString %>"
        ProviderName="<%$ ConnectionStrings:ConnectionString.ProviderName %>" SelectCommand="SELECT EmployeeID,   (FirstName + ' ' + LastName) AS FullName FROM TMs WHERE (EmployeeID = ?)">
        <SelectParameters>
            <asp:ControlParameter ControlID="TMListbyName" Name="?" PropertyName="SelectedValue" />
        </SelectParameters>
    </asp:SqlDataSource>
</asp:Content>
